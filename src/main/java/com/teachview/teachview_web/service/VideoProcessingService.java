...
public class VideoProcessingService {
        ...
        try {
            ...
            if (nvenc) {
                ...
                log.warn("VIDEO_ENCODER=gpu, но h264_nvenc недоступен в ffmpeg. " +
                    ...
                    "(nvidia-container-toolkit + docker-compose.gpu.yml), 3) на хосте установлены драйверы NVIDIA. " +
    ...
    private void transcribeVideo(Path videoFile, Path workDir) {
        // Запускаем оба варианта (расшифровка ru + перевод en) параллельно:
        // оба задания — это сетевые вызовы к Whisper API, поэтому одновременный
        // запуск даёт ~2× ускорение этого шага.
        CompletableFuture<Void> ruTask = CompletableFuture.runAsync(() ->
            runTranscriptionTask(videoFile, workDir.resolve("transcription.ru.txt"), "transcribe"));
        CompletableFuture<Void> enTask = CompletableFuture.runAsync(() ->
            runTranscriptionTask(videoFile, workDir.resolve("transcription.en.txt"), "translate"));
        try {
            CompletableFuture.allOf(ruTask, enTask).join();
        } catch (Exception e) {
            log.warn("Ошибка ожидания параллельной транскрипции: {}", e.getMessage(), e);
        }
    }
